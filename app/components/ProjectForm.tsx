...
export function ProjectForm({ destination }: { destination: "Belgique" | "Canada" }) {
  ...
  const [canadaProcedure, setCanadaProcedure] = useState("");
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok) { form.reset(); setContactMethod("whatsapp"); setCanadaProcedure(""); }
    } catch {
  ...
  return <form className="project-form" onSubmit={submit}>
    ...
    {destination === "Canada" ? <label className="full">Quelle procédure vous intéresse ?<select name="project" required value={canadaProcedure} onChange={(event) => setCanadaProcedure(event.target.value)}><option value="" disabled>Choisir une procédure</option><option value="Étudiant">Étudiant</option><option value="Travailleur">Travailleur</option><option value="Résidence permanente">Résidence permanente</option></select></label> : <input type="hidden" name="project" value="Études" />}
    <label className="full">Nom et prénom<input name="name" autoComplete="name" required minLength={2} maxLength={100} /></label>
    <>
    ...
    <label className="full">Niveau d’études actuel<input name="residence" required minLength={2} maxLength={120} placeholder="Ex. Terminale, licence, master…" /></label>
    {destination === "Canada" && canadaProcedure && canadaProcedure !== "Étudiant" && <label className="full">Dans quel domaine travaillez-vous actuellement ?<input name="currentWork" required minLength={2} maxLength={160} placeholder="Ex. Comptabilité, informatique, bâtiment…" /></label>}
    ...
    <button className="button button-primary full" disabled={status === "loading"}>{status === "loading" ? "Envoi en cours…" : "Envoyer"}<span>→</span></button>
