...
export async function POST(request: Request) {
  try {
    ...
    const usesPreferredContact = body.destination === "Belgique";
    ...
    if (!usesPreferredContact && (!body.email || !body.phone)) return Response.json({ message: "Merci de compléter tous les champs obligatoires." }, { status: 400 });
    if (!usesPreferredContact && !emailPattern.test(String(body.email))) return Response.json({ message: "L’adresse e-mail semble incorrecte." }, { status: 400 });
    ...
    const lines = [
      ...
      ["Résidence", body.residence],
      ["Destination", body.destination], ["Projet", body.project], ["Situation", body.situation], ["Échéance", body.timeline], ["Message", details || "Non renseigné"],
